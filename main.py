...
def _timed_stage(timings: dict[str, float], name: str):
    """Accumulate wall-clock timing for one import stage.

    Runtime telemetry is diagnostic only. It must never influence Data Guard,
    Ranking Guard, recovery decisions or Operational Truth.
    """
...
def main(argv=None):
    ...
    for screenshot in screenshots:
        ...
        with _timed_stage(runtime_timings, "image_load_normalize"):
        ...
        with _timed_stage(runtime_timings, "data_quality_loop"):
        ...
        with _timed_stage(runtime_timings, "row_parse"):
        ...
        if ranking_type == "total_hero_power":
            with _timed_stage(runtime_timings, "player_snapshot_build"):
    ...
    with _timed_stage(runtime_timings, "ranking_guard"):
        grouped = apply_ranking_guard(grouped)
    with _timed_stage(runtime_timings, "power_sanity_recovery"):
        grouped = apply_ranking_power_sanity_guard(grouped)
    with _timed_stage(runtime_timings, "adaptive_review_ocr"):
    ...
    with _timed_stage(runtime_timings, "server_reconciliation"):
